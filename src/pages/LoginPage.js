...
import { auth, setAuthPersistence, signInWithGoogle, signInWithGitHub, saveUserToFirestore } from '../firebase';
...
const LoginPage = ({ onLogin }) => {
  ...
  const handleForgotPassword = async () => {
    if (!email) {
      setMessage({ text: 'Enter your email address first', type: 'error' });
      return;
    }
    try {
  ...
  const handleGitHubSignIn = async () => {
    setMessage({ text: '', type: '' });
    try {
      const user = await signInWithGitHub(rememberMe);
      await saveUserToFirestore(user);
      onLogin(user.email);
      navigate('/');
    } catch (error) {
      if (error.code !== 'auth/popup-closed-by-user') {
        setMessage({ text: `GitHub sign‑in failed: ${error.message}`, type: 'error' });
      }
    }
  };
  ...
  const handleSocialLogin = (provider) => {
    if (provider === 'Google') {
      ...
      alert(`Logging in with ${provider} – not implemented.`);
    }
  };
